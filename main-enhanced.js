...
// Game engine FIXED - NO RELOAD LOOPS!
console.log('🎮 Game engine FIXED loaded - NO RELOAD LOOPS!');
...
class Game {
    ...
    updateUI() {
        ...
        document.getElementById('timeDisplay').textContent = this.world.getTimeString();
        const permTime = document.getElementById('permanentTimeDisplay');
            // Legg til touchend for mobile control buttons
            document.querySelectorAll('.control-button').forEach(btn => {
                if (!btn.hasTouchListener) {
    ...
    update(deltaTime) {
        ...
        if (this.player) {
            ...
            if (this.player.health <= 0) {
                console.log('🚨 Player died, calling exitToMainMenu');
                alert('Du døde! Spillet er over.');
                if (this.exitToMainMenu && typeof this.exitToMainMenu === 'function') {
                    this.exitToMainMenu();
                } else {
                    console.error('exitToMainMenu method not found!');
                }
            }
            if (this.world && this.world.currentDay > 99) {
                console.log('🎉 Player survived 99 days!');
            ...
            if (this.isPaused) {
                ...
                pauseMenu.innerHTML = `
                    <div class="pause-content" style="max-width: 85vw; overflow-x: hidden; text-align: center;">
                        <h2 style="font-size: 1.3em; margin-bottom: 20px;">⏸️ v4.0 Fungerer!</h2>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function robustMenuButtonEvents() {
        ...
        buttons.forEach(btn => {
            ...
            // Add click, touchend, and touchstart listeners
    ...
    window.game.hideMobileHelp = function() {
        const overlay = document.getElementById('mobileHelpOverlay');
        if (overlay) overlay.style.display = 'none';
        const canvas = document.getElementById('gameCanvas');
        if (canvas) canvas.classList.remove('ui-block');
    ...
    window.addEventListener('orientationchange', () => {
        setTimeout(() => {
            const canvas = document.getElementById('gameCanvas');
            if (canvas) {
                // Bruk devicePixelRatio for skarphet
    ...
    function setMenuFunctions() {
        window.game.startNewGame = function() {
            ...
            robustMenuButtonEvents(); // Ensure robust listeners are added after new game
        };
                ...
                pauseMenu.innerHTML = `
                    <div class="pause-content" style="max-width: 85vw; max-height: 80vh; overflow: auto; padding: 15px; word-wrap: break-word;">
                        ...
                        <div style="font-size: 0.8em; line-height: 1.3; text-align: left;">
                            <p style="margin: 0 0 8px 0; color: #4CAF50;">✅ Oppdatert v4!</p>
    ...
    try {
        ...
        // Skjul loading screen selv om det oppstår feil
        const loadingScreen = document.getElementById('loadingScreen');
        if (loadingScreen) {
            loadingScreen.style.opacity = '0';
            loadingScreen.style.visibility = 'hidden';
            ...
            loadingScreen.style.pointerEvents = 'none';
            loadingScreen.style.zIndex = '-1';
            console.log('❌ Loading screen skjult etter feil');
        }
    }
});